...
<script type="text/javascript">
				...
				<tbody>
					...
					<tr>
						<td colspan="6" align=center>
							...
							onClick="location.href='list_Proceeding?curBlock=<%=curBlock - 1%>'"
							value="이전"> <%
 	} else {
 %> <input type="button" class="btn btn-default disabled"
 		...
 		if (curBlock > 0) {
 %> <input type="button" class="btn btn-default"
							onClick="location.href='list_Proceeding?search=<%=search%>&search_text=<%=search_text%>&curBlock=<%=curBlock - 1%>'"
							value="이전"> <%
 	} else {
 %> <input type="button" class="btn btn-default disabled"
							onClick="location.href='list_Proceeding?search=<%=search%>&search_text=<%=search_text%>&curBlock=<%=curBlock - 1%>'"
 ...
 %> <%
 	...
 	for (int i = startPage; i < endPage; i++) {
 		...
 		if (search == null || search_text == null) {
 %> <input type="button" class="btn btn-default"
							onClick="location.href='list_Proceeding?curBlock=<%=curBlock%>&curPage=<%=i%>'"
							value="<%=i + 1%>"> <%
 	} else {
 %> <input type="button" class="btn btn-default"
							onClick="location.href='list_Proceeding?search=<%=search%>&search_text=<%=search_text%>&curBlock=<%=curBlock%>&curPage=<%=i%>'"
							value="<%=i + 1%>"> <%
 	}
 	}
 ...
 %>
		...
		<br>
<form action="list_Proceeding" method="post">
				<%
					if (position_num >= 3) {
				%>
