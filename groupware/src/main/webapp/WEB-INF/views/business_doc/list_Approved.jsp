...
<body>
	...
	<br>
	<div class="container">
		<br> <a href="list_business?member_id=<%=member_id%>"
			class="btn btn-default"
			style="background-color: #3E454C; color: #FCFCFC;">업무일지 전체보기</a><br>

		<br>
		<h4>업무일지 승인완료(최신순으로 게시)</h4>
				...
				<tbody>
					...
					<c:forEach items="${list2}" var="dto" begin="<%=beginNum%>"
						...
						<tr>
							...
							<td><a href="approved_View?business_num=${dto.business_num}"
								style="color: black; text-decoration: none;">${dto.business_title}</a></td>

							<td>${dto.member_name}</td>
							<td>${dto.business_date}</td>
							<td>${dto.approved_date}</td>
							<td>
								<button class="btn btn-default btn-sm"
									style="background-color: #00C3FF; color: #FCFCFC;">${dto.condition_name }</button>
							</td>
							<td>
								<button type="button" class="btn btn-default btn-xs"
									onclick="location.href='approved_View?business_num=${dto.business_num}'">
					...
					<tr>
						<td colspan="6" align=center>
							...
							onClick="location.href='list_Approved?curBlock=<%=curBlock - 1%>'"
							value="이전"> <%
 	} else {
 %> <input type="button" class="btn btn-default disabled"
 		...
 		if (curBlock > 0) {
 %> <input type="button" class="btn btn-default"
							onClick="location.href='list_Approved?search=<%=search%>&search_text=<%=search_text%>&curBlock=<%=curBlock - 1%>'"
							value="이전"> <%
 	} else {
 %> <input type="button" class="btn btn-default disabled"
							onClick="location.href='list_Approved?search=<%=search%>&search_text=<%=search_text%>&curBlock=<%=curBlock - 1%>'"
 ...
 %> <%
 	...
 	for (int i = startPage; i < endPage; i++) {
 		...
 		if (search == null || search_text == null) {
 %> <input type="button" class="btn btn-default"
							onClick="location.href='list_Approved?curBlock=<%=curBlock%>&curPage=<%=i%>'"
							value="<%=i + 1%>"> <%
 	} else {
 %> <input type="button" class="btn btn-default"
							onClick="location.href='list_Approved?search=<%=search%>&search_text=<%=search_text%>&curBlock=<%=curBlock%>&curPage=<%=i%>'"
							value="<%=i + 1%>"> <%
 	}
 	}
 ...
 %>
			...
			<form action="list_Approved" method="post">
				<%
					if (position_num >= 3) {
				%>
